...
# Gerar posições iniciais dos obstáculos garantindo que não colidam com o jogador ou entre si
posicoesObstaculos = []
for _ in range(numObstaculos):
    while True:
        ...
        if not colide(posicaoObstaculo, tamanhoAlvo, posicaoJogador, tamanhoJogador):
            colisao = False
            for po in posicoesObstaculos:
                if colide(posicaoObstaculo, tamanhoAlvo, po, tamanhoAlvo):
                    colisao = True
                    break
            if not colisao:
